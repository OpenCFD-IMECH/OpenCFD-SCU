...
void opencfd_mem_init_Stream(){
    for (int i = 0; i < 4; i++) hipStreamCreate(&Stream[i]);
    for (int i = 0; i < 4; i++) hipEventCreate(&Event[i]);
}

void opencfd_mem_finalize_Stream(){
    for (int i = 0; i < 4; ++i) hipStreamDestroy(Stream[i]);
...
void *du_Jacobian3d_all(void* pthread_id){
    ...
    hipEventRecord(Event[1], Stream[1]);//记录数据交换情况
    hipStreamWaitEvent(Stream[2], Event[1], 0);//外区等待粘性计算完
    ...
    du_viscous_Jacobian3d_init(&Stream[2]);//开始算粘性项全体导数
    du_viscous_Jacobian3d_x_init(&Stream[2]);//粘性项计算
    hipEventRecord(Event[2], Stream[2]);//记录粘性计算
    du_vis_Jacobian3d_inner_x(&Stream[2]);//内区开始计算

    hipStreamWaitEvent(Stream[3], Event[1], 0);//外区等待粘性计算完
    hipStreamWaitEvent(Stream[3], Event[2], 0);//外区等待粘性计算完
    du_vis_Jacobian3d_outer_x(&Stream[3]);//外区x计算
    hipEventRecord(Event[3], Stream[3]);

    hipStreamWaitEvent(Stream[1], Event[3], 0);
    du_invis_Jacobian3d_outer_y(&Stream[1]);
    hipEventRecord(Event[1], Stream[1]);

    hipStreamWaitEvent(Stream[2], Event[3], 0);
    du_viscous_Jacobian3d_y_init(&Stream[2]);
    hipEventRecord(Event[2], Stream[2]);//记录粘性计算
    du_vis_Jacobian3d_inner_y(&Stream[2]);//内区开始计算
