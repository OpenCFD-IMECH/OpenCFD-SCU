...
void OCFD_Dx0_bound(cudaField f , cudaField fx , cudaJobPackage job_in , dim3 blockdim_in, hipStream_t *stream){ 
	CHECK_X(
		{
			CUDA_LAUNCH(( hipLaunchKernelGGL(OCFD_Dx0_bound_kernel_m, dim3(griddim ), dim3(blockdim), 0, *stream, f, fx, job_in) ));

		},{
			CUDA_LAUNCH(( hipLaunchKernelGGL(OCFD_Dx0_bound_kernel_p, dim3(griddim ), dim3(blockdim), 0, *stream, f, fx, job_in) ));
		}
	)
}
...
void OCFD_Dy0_bound(cudaField f , cudaField fx , cudaJobPackage job_in , dim3 blockdim_in, hipStream_t *stream){
	CHECK_Y(
		{
			CUDA_LAUNCH(( hipLaunchKernelGGL(OCFD_Dy0_bound_kernel_m, dim3(griddim ), dim3(blockdim), 0, *stream, f, fx, job_in) ));
		},
		{
			CUDA_LAUNCH(( hipLaunchKernelGGL(OCFD_Dy0_bound_kernel_p, dim3(griddim ), dim3(blockdim), 0, *stream, f, fx, job_in) ));
		}
	)
}
...
void OCFD_Dz0_bound(cudaField f , cudaField fx , cudaJobPackage job_in , dim3 blockdim_in, hipStream_t *stream){
	CHECK_Z(
		{
			CUDA_LAUNCH(( hipLaunchKernelGGL(OCFD_Dz0_bound_kernel_m, dim3(griddim ), dim3(blockdim), 0, *stream, f, fx, job_in) ));
		},
		{
			CUDA_LAUNCH(( hipLaunchKernelGGL(OCFD_Dz0_bound_kernel_p, dim3(griddim ), dim3(blockdim), 0, *stream, f, fx, job_in) ));
		}
	)
}
