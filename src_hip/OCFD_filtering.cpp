	...
	int IF_Filter)
{
	if(IF_Filter == 1) 
	{
		...
		CUDA_LAUNCH(( hipLaunchKernelGGL(filter_x3d_shock_kernel, dim3(griddim ), dim3(blockdim ), sizeof(REAL)*(blockdim.x) * blockdim.y * blockdim.z, 0, *pP_d, *pf_lap_d, *pf_d, rth, s0, job) ));
	}
}
	...
	int IF_Filter)
{
	if(IF_Filter == 1){
		...
		CUDA_LAUNCH((hipLaunchKernelGGL(filter_y3d_shock_kernel, dim3(griddim), dim3(blockdim), sizeof(REAL)*(blockdim.x)*blockdim.y*blockdim.z, 0, *pP_d, *pf_lap_d, *pf_d, rth, s0, job)));
	}
}
